...
DEPLOY_NOTIFY_PROMPT = (
    "Post a Jira-aware deploy notification in Slack.\n"
    "  Channel ID: {channel}\n"
    "  Tag these users (resolve each via slack_search_users): {users}\n"
    "  Env: {env}\n"
    "  Deploy URL: {deploy_url}\n"
    "  PR branch: {branch}\n\n"
    "Step 1 — Find the Jira ticket key.\n"
    "Scan the PR branch name for a Jira key matching the pattern [A-Z]+-[0-9]+ "
    "(e.g. CGP-46, CSI-1812). If none is found, STOP and reply with exactly:\n"
    "  ERROR: No Jira ticket key in branch name '{branch}'\n"
    "Do not post anything to Slack in that case.\n\n"
    "Step 2 — Look up the ticket.\n"
    "Call mcp__claude_ai_Atlassian__getJiraIssue with the cloudId for "
    "cognota.atlassian.net (use mcp__claude_ai_Atlassian__getAccessibleAtlassianResources "
    "if you don't already have it) and the ticket key. Read:\n"
    "  - issuetype.name on the ticket itself\n"
    "  - parent.key and parent.fields.issuetype.name (may be absent)\n\n"
    "Step 3 — Pick the effective ticket, then the message template.\n"
    "Let TICKET_URL = https://cognota.atlassian.net/browse/<ticket key>\n"
    "Let PARENT_URL = https://cognota.atlassian.net/browse/<parent key>\n"
    "First pick the EFFECTIVE ticket — the unit of work being deployed. Always "
    "roll up to the parent, but stop at an Epic:\n"
    "  - If a parent exists AND the parent issuetype is NOT 'Epic' →\n"
    "    EFFECTIVE_TYPE = parent issuetype, EFFECTIVE_URL = PARENT_URL\n"
    "  - Otherwise (no parent, or the parent issuetype is 'Epic') →\n"
    "    EFFECTIVE_TYPE = ticket issuetype, EFFECTIVE_URL = TICKET_URL\n"
    "Never announce an Epic itself — an Epic is only a container, never the "
    "EFFECTIVE ticket.\n"
    "Then pick wording from EFFECTIVE_TYPE:\n"
    "  a. If EFFECTIVE_TYPE is 'Bug', 'Defect', or 'Hotfix' →\n"
    "     subject = \"this bug fix\", link = EFFECTIVE_URL\n"
    "  b. Otherwise →\n"
    "     subject = \"this story\", link = EFFECTIVE_URL\n\n"
    "Step 4 — Resolve Slack users.\n"
    "Resolve each user in the tag list via slack_search_users (try transformations "
    "like dropping trailing digits or swapping `_` for `.` if the first attempt "
    "misses). Skip users who don't resolve and note them in your reply.\n\n"
    "Step 5 — Send EXACTLY ONE Slack message via slack_send_message to the channel "
    "ID above. Message body must be:\n"
    "  <@USER_ID_1> <@USER_ID_2> I've deployed <subject> <link> to the {env} "
    "instance for testing, can you please take a look\n"
    "  {deploy_url}\n\n"
    "Rules:\n"
